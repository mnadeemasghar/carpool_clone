        ...
        <div class="d-flex justify-content-between align-items-center mt-3">
            ...
            <div>
                @if ($ride->isUserInterested(auth()->id()))
                    <form target="_blank" action="{{ route('ride.interested', ['ride' => $ride]) }}" method="POST" style="display:inline;">
                        @csrf
                ...
                @endif
                <a href="{{ route('ride.show', ['ride' => $ride]) }}" target="_blank" class="btn btn-outline-primary btn-sm">View Details</a>
            </div>
        </div>
